@props(['href', 'active' => false, 'icon' => 'grid', 'label', 'badge' => null])
...
@php
    $icons = [
        ...
        'user-group' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>',
        'star' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.175 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.382-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>',
        ...
        'gamepad' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"></path>',
...
<a href="{{ $href }}"
   ...
   {{ $active
      ? 'bg-gradient-to-r from-yellow-500 to-yellow-400 text-black font-black shadow-lg shadow-yellow-500/30'
      : 'text-gray-400 hover:text-white hover:bg-gray-800 hover:translate-x-1'
   }}">
    <div class="flex items-center gap-3">
        <div class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors
            {{ $active ? 'bg-black/10' : 'bg-gray-800 group-hover:bg-yellow-500/10 group-hover:text-yellow-500' }}">
    ...
    @if($badge)
        <span class="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest
            {{ $active ? 'bg-black/20 text-black' : 'bg-yellow-500 text-black shadow-sm shadow-yellow-500/20' }}">
            {{ $badge }}
        </span>
    @elseif(!$active)
